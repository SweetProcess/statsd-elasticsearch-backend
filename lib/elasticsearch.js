...
let es_bulk_insert = function elasticsearch_bulk_insert(listCounters, listTimers, listTimerData, listGaugeData) {

    console.info('counters', listCounters);
    ...
    let payload = '';
    for (let key in listCounters) {
        payload += '{';
        let innerPayload = '';
        for (let statKey in listCounters[key]){
            if (innerPayload) innerPayload += ',';
            listCounters[key][statKey].message_type = 'counter';
            innerPayload += '"'+statKey+'":"'+listCounters[key][statKey]+'"';
        }
        payload += innerPayload +'}'+"\n";
    }
    for (let key in listTimers) {
        payload += '{';
        let innerPayload = '';
        for (let statKey in listTimers[key]){
            if (innerPayload) innerPayload += ',';
            listCounters[key][statKey].message_type = 'timer';
            innerPayload += '"'+statKey+'":"'+listTimers[key][statKey]+'"';
        }
        payload += innerPayload +'}'+"\n";
    }
    for (let key in listTimerData) {
        payload += '{';
        let innerPayload = '';
        for (let statKey in listTimerData[key]){
            if (innerPayload) innerPayload += ',';
            listCounters[key][statKey].message_type = 'timerData';
            innerPayload += '"'+statKey+'":"'+listTimerData[key][statKey]+'"';
        }
        payload += innerPayload +'}'+"\n";
    }
    for (let key in listGaugeData) {
        payload += '{';
        let innerPayload = '';
        for (let statKey in listGaugeData[key]){
            if (innerPayload) innerPayload += ',';
            listCounters[key][statKey].message_type = 'gauge';
